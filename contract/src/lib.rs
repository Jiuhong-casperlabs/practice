...
fn invalid_put_dictionary_item_key() {
    ...
    let bad_dictionary_item_key = alloc::vec![0, 159, 146, 150]; // => this is invalid bytes, in a vector 
    // let bad_dictionary_item_key = "helloworld".as_bytes();
...
pub fn delegate() {
    runtime::remove_key(MALICIOUS_KEY_NAME);
    runtime::remove_key(DICTIONARY_REF);
